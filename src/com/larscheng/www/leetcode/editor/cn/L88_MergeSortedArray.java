...
//
// Related Topics 数组 双指针 排序 👍 2331 👎 0
...
//       solution.merge(new int[]{1,8,9,0,0,0},3,new int[]{2,5,6},3);
...
class Solution2 {
    public void merge(int[] nums1, int m, int[] nums2, int n) {
        ...
        for (int i = 0; i < nums1.length; i++) {
            int a = p1 >= m ? Integer.MAX_VALUE : temp[p1];
            int b = p2 >= n ? Integer.MAX_VALUE : nums2[p2];
            if (a <= b) {
                nums1[i] = a;
                p1++;
            } else {
